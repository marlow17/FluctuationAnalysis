function DFAmoreExamples(n,duration,numOfTrials,fs)
% Some more examples illustrating DFA vis-a-vis DFA + model selection...
%
...
for i=n
    ...
    %% conventional SDA over the entire range - this suits example 1
    subplot(1,4,2);
    % define the fitting range
    interval2fit=interval2evaluate;
    % run the analysis (including text & graphics report)
    fluctuationAnalysis(y,interval2evaluate,interval2fit,'SDA','verbose','graphics','fs',fs);
    title('SDA over full range'); set(gca,'xlim',fullrange); drawnow;

    %% DFA including (Bayesian) parametric model selection - this hardly suits any example
    subplot(1,4,3);
    % define the fitting range
    interval2fit=interval2evaluate;
    % run the analysis (including text & graphics report)
